...
export async function getNFTs(
  ...
  const lockedNFTs = await publicClient.multicall({
  ...
  console.log(`multicall locked took ${t2 - t1}ms`)

  const data = nfts.map((nft, index) => ({
    id: nft,
    amount: formatUnits(lockedNFTs[index][0], 18),
    end: Number(lockedNFTs[index][1]),
  }))
  return data
    .filter((d) => d.amount !== '0')
    .reduce(
...
export function writeJson(data, fileName) {
  fs.writeFileSync('./src/data/' + fileName, JSON.stringify(data))
}
